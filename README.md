...
DevOps Research & Assessment (DORA) metrics is now the standard term for these metrics.
...
- Run the script to generate the metrics, passing in:
  ...
  - WindowIntervalDays -- The interval (in days) between each point in the graph (defaults to 7) RedGate Metrics supports other parameters but the above list will do for a start.
...
Only Deploy releases should be considered for Lead Time and Deployment Frequency. What do we do with MTTR and Change Fail? Should that include configuration changes as well? Maybe the thing to do is to model this as a stream of events:

- SuccessfulDeploy
- FailedDeploy
- ServiceIncident
- ServiceRestored The tricky part is the connection between Deploy and ServiceIncident. Maybe FailedDeploy doesn't need to exist if we can use the sequence of Deploy and ServiceIncident to infer that the change failed or link it some other way.
